...
deployment_name = "agent-sandbox-controller"
namespace_name = "agent-sandbox-system"
...
def deployment(request, kube_cluster: Cluster) -> List[pykube.Deployment]:
    logger.info("Waiting for agent-sandbox-controller deployment..")

    deployment_ready = wait_for_deployment(kube_cluster)

    logger.info("agent-sandbox-controller deployment looks satisfied..")
